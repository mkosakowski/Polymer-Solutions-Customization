report 50106 "RSMUSExportElectronicPayments"
{
    //9938 BRD-0026 REP # 10083 C.M.H. M.S
    //Custom report to add footer and posting date field
    DefaultLayout = RDLC;
    RDLCLayout = './src/Report/Layouts/Rep50106.RSMUSExportElectronicPayments.rdl';
        ...
        dataitem("Gen. Journal Line"; "Gen. Journal Line")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(mypostingdate; Format("Gen. Journal Line"."Posting Date", 0, '<Month,2>/<Day,2>/<Year4>'))
                    {
                    }
                    ...
                    dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                    {
                        ...
                        begin
                            ...
                            begin
                                DiscountTaken:=0;
                                if(-ExportAmount - TotalAmountPaid) > -"Amount to Apply" then AmountPaid:=-"Amount to Apply"
                                else
 ...
 Clear(CopyTxt)
            ...
            begin
                ...
                if "Account Type" = "Account Type"::"Bank Account" then begin
                    ...
                    if "Account No." <> BankAccount."No." then CurrReport.Skip();
                end
                else if "Bal. Account Type" = "Bal. Account Type"::"Bank Account" then begin
                        BankAccountIs:=BankAccountIs::BalAcnt;
                        if "Bal. Account No." <> BankAccount."No." then CurrReport.Skip();
                    end
                    else
                        CurrReport.Skip();
                if BankAccountIs = BankAccountIs::Acnt then begin
                    ExportAmount:="Amount (LCY)";
                    case "Bal. Account Type" of "Bal. Account Type"::Vendor: begin
                ...
                begin
                    ExportAmount:=-"Amount (LCY)";
                    case "Account Type" of "Account Type"::Vendor: begin
                        PayeeType:=PayeeType::Vendor;
                ...
                AmountPaid:=0;
                TotalAmountPaid:=0;
                if PayeeType = PayeeType::Vendor then ProcessVendor("Gen. Journal Line")
                else
    ...
    begin
        ...
        if Filter = '' then begin
            ...
            Filter:="Gen. Journal Line".GetFilter("Journal Template Name")end;
        GenJournalTemplate.Get(Filter);
        if not UseRequestPage()then if "Gen. Journal Line".FindFirst()then if GenJournalBatch.Get("Gen. Journal Line"."Journal Template Name", "Gen. Journal Line"."Journal Batch Name")then if GenJournalBatch."Bal. Account Type" = GenJournalBatch."Bal. Account Type"::"Bank Account" then BankAccount."No.":=GenJournalBatch."Bal. Account No.";
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            if(VendLedgEntry."Pmt. Discount Date" >= GenJnlLine."Document Date") and (VendLedgEntry."Remaining Pmt. Disc. Possible" <> 0) and (-(ExportAmount + VendLedgEntry."Remaining Pmt. Disc. Possible") >= -VendLedgEntry."Amount to Apply")then begin
            ...
            else if -ExportAmount > -VendLedgEntry."Amount to Apply" then AmountPaid:=-VendLedgEntry."Amount to Apply"
                else
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            if(CustLedgEntry."Pmt. Discount Date" >= GenJnlLine."Document Date") and (CustLedgEntry."Remaining Pmt. Disc. Possible" <> 0) and (-(ExportAmount - CustLedgEntry."Remaining Pmt. Disc. Possible") >= -CustLedgEntry."Amount to Apply")then begin
            ...
            else if -ExportAmount > -CustLedgEntry."Amount to Apply" then AmountPaid:=-CustLedgEntry."Amount to Apply"
                else
                    AmountPaid:=-ExportAmount;
        end;
    end;

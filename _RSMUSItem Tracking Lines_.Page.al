page 50108 "RSMUSItem Tracking Lines"
{
            ...
            repeater(Control1)
            {
                ...
                field("Serial No."; Rec."Serial No.")
                {
                    ...
                    begin
                        ...
                        if(Rec."Source Type" = DATABASE::"Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass)then ItemTrackingDataCollection.SetDirectTransfer(true);
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec, (CurrentSignFactor * SourceQuantityArray[1] < 0) and not InsertIsBlocked, CurrentSignFactor, "Item Tracking Type"::"Serial No.", MaxQuantity);
                        Rec."Bin Code":='';
                        CurrPage.Update();
                    end;
                    trigger OnValidate()
                ...
                field("Lot No."; Rec."Lot No.")
                {
                    ...
                    begin
                        ...
                        Rec."Bin Code":=ForBinCode;
                        if(Rec."Source Type" = DATABASE::"Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass)then ItemTrackingDataCollection.SetDirectTransfer(true);
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec, (CurrentSignFactor * SourceQuantityArray[1] < 0) and not InsertIsBlocked, CurrentSignFactor, "Item Tracking Type"::"Lot No.", MaxQuantity);
                        Rec."Bin Code":='';
                ...
                field("Package No."; Rec."Package No.")
                {
                    ...
                    begin
                        ...
                        Rec."Bin Code":=ForBinCode;
                        if(Rec."Source Type" = DATABASE::"Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass)then ItemTrackingDataCollection.SetDirectTransfer(true);
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec, (CurrentSignFactor * SourceQuantityArray[1] < 0) and not InsertIsBlocked, CurrentSignFactor, "Item Tracking Type"::"Package No.", MaxQuantity);
                        Rec."Bin Code":='';
                        CurrPage.Update();
                    end;
    ...
    begin
        ...
        if UpdateUndefinedQty() and not SkipWriteToDatabase then WriteToDatabase();
        if CurrentRunMode = CurrentRunMode::"Drop Shipment" then case CurrentSourceType of DATABASE::"Sales Line": SynchronizeLinkedSources(StrSubstNo(Text015, Text016));
            DATABASE::"Purchase Line": SynchronizeLinkedSources(StrSubstNo(Text015, Text017));
            end;
        if(CurrentRunMode = CurrentRunMode::Transfer) or IsOrderToOrderBindingToTransfer()then SynchronizeLinkedSources('');
    ...
    begin
        ...
        if not DeleteIsBlocked then begin
            ...
            Rec.Delete(true);
            if not AlreadyDeleted then TempItemTrackLineDelete.Insert();
            ItemTrackingDataCollection.UpdateTrackingDataSetWithChange(TempItemTrackLineDelete, CurrentSignFactor * SourceQuantityArray[1] < 0, CurrentSignFactor, 2);
            if TempItemTrackLineInsert.Get(Rec."Entry No.")then TempItemTrackLineInsert.Delete();
            if TempItemTrackLineModify.Get(Rec."Entry No.")then TempItemTrackLineModify.Delete();
        end;
        CalculateSums();
        exit(false);
    end;
    ...
    trigger OnInsertRecord(BelowxRec: Boolean)Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        OnBeforeOnInsertRecord(Rec, SourceQuantityArray, Result, IsHandled);
        if IsHandled then exit(Result);
        if Rec."Entry No." <> 0 then exit(false);
        InsertRecord(Rec);
        CalculateSums();
        exit(false);
    end;
    ...
    begin
        ...
        if IsHandled then exit(Result);
        if InsertIsBlocked then if not Rec.HasSameTracking(xRec) or (xRec."Quantity (Base)" <> Rec."Quantity (Base)")then exit(false);
        UpdateTrackingData();
        CalculateSums();
        exit(false);
    end;
    ...
    trigger OnQueryClosePage(CloseAction: Action): Boolean var
        IsHandled: Boolean;
    begin
        if not UpdateUndefinedQty()then exit(Confirm(Text006));
    ...
    ItemTrackingDataCollection: Codeunit "RSMUSItemTrackingDataCollect";
    ...
    begin
        ...
        if(not InsertIsBlocked) and (not ZeroLineExists())then if not TestTempSpecificationExists()then begin
...
#if not CLEAN19[Obsolete('Replaced by SetRynMode().', '19.0')]
...
#endif procedure SetRunMode(RunMode: Enum "Item Tracking Run Mode")
    begin
        CurrentRunMode:=RunMode;
    end;
    procedure GetRunMode(): Enum "Item Tracking Run Mode" begin
        exit(CurrentRunMode);
    end;
#if not CLEAN19[Obsolete('Replaced by GetRunMode().', '19.0')]
    procedure GetFormRunMode(var Mode: Option, Reclass, "Combined Ship/Rcpt", "Drop Shipment", Transfer)
    begin
        Mode:=CurrentRunMode.AsInteger();
    end;
#endif protected procedure UpdateTrackingData()
    var
    ...
    begin
        ...
        if not TestTempSpecificationExists()then begin
            ...
            if not Rec.HasSameTracking(xRec)then begin
                ...
                ItemTrackingDataCollection.UpdateTrackingDataSetWithChange(TempTrackingSpec, CurrentSignFactor * SourceQuantityArray[1] < 0, CurrentSignFactor, 2);
            end;
            if TempItemTrackLineModify.Get(Rec."Entry No.")then TempItemTrackLineModify.Delete();
            if TempItemTrackLineInsert.Get(Rec."Entry No.")then begin
                ...
                ItemTrackingDataCollection.UpdateTrackingDataSetWithChange(TempItemTrackLineInsert, CurrentSignFactor * SourceQuantityArray[1] < 0, CurrentSignFactor, 1);
            end
            else
            begin
                TempItemTrackLineModify.TransferFields(Rec);
                TempItemTrackLineModify.Insert();
                ItemTrackingDataCollection.UpdateTrackingDataSetWithChange(TempItemTrackLineModify, CurrentSignFactor * SourceQuantityArray[1] < 0, CurrentSignFactor, 1);
            end;
        end;
    end;
    procedure SetSourceSpec(TrackingSpecification: Record "Tracking Specification"; AvailabilityDate: Date): Code[50]var
    ...
    begin
        ...
        LastEntryNo:=0;
        if ItemTrackingMgt.IsOrderNetworkEntity(TrackingSpecification."Source Type", TrackingSpecification."Source Subtype") and not(CurrentRunMode = CurrentRunMode::"Drop Shipment")then CurrentEntryStatus:=CurrentEntryStatus::Surplus
        else
            CurrentEntryStatus:=CurrentEntryStatus::Prospect;
        if(TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass)then CurrentEntryStatus:=CurrentEntryStatus::Prospect;
        CurrentEntryStatusOption:=CurrentEntryStatus.AsInteger();
        OnSetSourceSpecOnAfterAssignCurrentEntryStatus(TrackingSpecification, CurrentEntryStatusOption, ItemTrackingCode, InsertIsBlocked);
        ...
        if CurrentRunMode = CurrentRunMode::"Combined Ship/Rcpt" then SetPageControls("Item Tracking Lines Controls"::Tracking, false);
        ...
        ReservEntry.SetSourceFilter(TrackingSpecification."Source Type", TrackingSpecification."Source Subtype", TrackingSpecification."Source ID", TrackingSpecification."Source Ref. No.", true);
        ReservEntry.SetSourceFilter(TrackingSpecification."Source Batch Name", TrackingSpecification."Source Prod. Order Line");
        ...
        if TrackingSpecification.FindSet()then repeat TempTrackingSpecification:=TrackingSpecification;
                TempTrackingSpecification.Insert();
            until TrackingSpecification.Next() = 0;
        // Data regarding posted quantities on transfers is collected from Item Ledger Entries:
        if TrackingSpecification."Source Type" = DATABASE::"Transfer Line" then CollectPostedTransferEntries(TrackingSpecification, TempTrackingSpecification);
        // Data regarding posted quantities on assembly orders is collected from Item Ledger Entries:
        if not ExcludePostedEntries then if(TrackingSpecification."Source Type" = DATABASE::"Assembly Line") or (TrackingSpecification."Source Type" = DATABASE::"Assembly Header")then CollectPostedAssemblyEntries(TrackingSpecification, TempTrackingSpecification);
        // Data regarding posted output quantities on prod.orders is collected from Item Ledger Entries:
        if TrackingSpecification."Source Type" = DATABASE::"Prod. Order Line" then if TrackingSpecification."Source Subtype" = 3 then CollectPostedOutputEntries(TrackingSpecification, TempTrackingSpecification);
        // If run for Drop Shipment a RowID is prepared for synchronisation:
        if CurrentRunMode = CurrentRunMode::"Drop Shipment" then CurrentSourceRowID:=ItemTrackingMgt.ComposeRowID(TrackingSpecification."Source Type", TrackingSpecification."Source Subtype", TrackingSpecification."Source ID", TrackingSpecification."Source Batch Name", TrackingSpecification."Source Prod. Order Line", TrackingSpecification."Source Ref. No.");
        ...
        if(TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and (TrackingSpecification."Source Subtype" = 0)then begin
            BlockCommit:=true;
            CurrentSourceRowID:=ItemTrackingMgt.ComposeRowID(TrackingSpecification."Source Type", TrackingSpecification."Source Subtype", TrackingSpecification."Source ID", TrackingSpecification."Source Batch Name", TrackingSpecification."Source Prod. Order Line", TrackingSpecification."Source Ref. No.");
            SecondSourceRowID:=ItemTrackingMgt.ComposeRowID(TrackingSpecification."Source Type", 1, TrackingSpecification."Source ID", TrackingSpecification."Source Batch Name", TrackingSpecification."Source Prod. Order Line", TrackingSpecification."Source Ref. No.");
        ...
        lLotNo:=ItemTrackingDataCollection.RetrieveLookupData(Rec, false);
        FunctionsDemandVisible:=CurrentSignFactor * SourceQuantityArray[1] < 0;
        FunctionsSupplyVisible:=not FunctionsDemandVisible;
        OnAfterSetSourceSpec(TrackingSpecification, Rec, AvailabilityDate, BlockCommit, FunctionsDemandVisible, FunctionsSupplyVisible, QtyToHandleBaseEditable, QuantityBaseEditable, InsertIsBlocked);
        exit(lLotNo);
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        if(TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and (CurrentRunMode <> CurrentRunMode::Transfer) and (TrackingSpecification."Source Subtype" = 1)then begin
    ...
    begin
        ...
        case SecondSourceQuantityArray[1]of DATABASE::"Warehouse Receipt Line", DATABASE::"Warehouse Shipment Line": begin
    ...
    var
        ...
        AddTracking: Boolean;
    begin
        if ReservEntry.FindSet()then repeat if Color = 0 then begin
                    TempReservEntry:=ReservEntry;
                    TempReservEntry.Insert();
                end;
                if ReservEntry.TrackingExists()then begin
                    AddTracking:=true;
                    if SecondSourceID = DATABASE::"Warehouse Shipment Line" then if FromReservEntry.Get(ReservEntry."Entry No.", not ReservEntry.Positive)then AddTracking:=(FromReservEntry."Source Type" = DATABASE::"Assembly Header") = IsAssembleToOrder
                        else
                    ...
                    if AddTracking then begin
                        ...
                        if SwapSign then TempTrackingSpecification."Quantity (Base)"*=-1;
    ...
    begin
        ...
        if TempTrackingSpecification.Find('-')then repeat TempTrackingSpecification.SetTrackingFilterFromSpec(TempTrackingSpecification);
                TempTrackingSpecification.CalcSums("Quantity (Base)", "Qty. to Handle (Base)", "Qty. to Invoice (Base)", "Quantity Handled (Base)", "Quantity Invoiced (Base)");
                ...
                if TempTrackingSpecification."Quantity (Base)" <> 0 then begin
                    ...
                    if not ItemTrackingCode."Use Expiration Dates" then Rec."Buffer Status2":=Rec."Buffer Status2"::"ExpDate blocked"
                    else
                    begin
...
#if not CLEAN20[Obsolete('Replaced by SetPageControls().', '19.0')]
...
#endif protected procedure SetPageControls(Controls: Enum "Item Tracking Lines Controls"; SetAccess: Boolean)
    begin
        case Controls of "Item Tracking Lines Controls"::Handle: begin
        ...
        "Item Tracking Lines Controls"::Invoice: begin
        ...
        "Item Tracking Lines Controls"::Quantity: begin
        ...
        "Item Tracking Lines Controls"::Reclass: begin
        ...
        "Item Tracking Lines Controls"::Tracking: begin
            SerialNoEditable:=SetAccess;
            LotNoEditable:=SetAccess;
            PackageNoEditable:=SetAccess;
    ...
    begin
        ...
        if IsHandled then exit;
        if ItemTrackingMgt.ItemTrkgIsManagedByWhse(TrackingSpecification."Source Type", TrackingSpecification."Source Subtype", TrackingSpecification."Source ID", TrackingSpecification."Source Prod. Order Line", TrackingSpecification."Source Ref. No.", TrackingSpecification."Location Code", TrackingSpecification."Item No.")then begin
            SetPageControls("Item Tracking Lines Controls"::Quantity, false);
            QtyToHandleBaseEditable:=true;
            DeleteIsBlocked:=true;
    ...
    begin
        if Item."No." <> ItemNo then begin
            ...
            if ItemTrackingCode.Code <> Item."Item Tracking Code" then ItemTrackingCode.Get(Item."Item Tracking Code");
        end;
    end;
    ...
    begin
        ...
        if(TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and (TrackingSpecification."Source Subtype" = 1)then begin
    ...
    begin
        ...
        if IsHandled then exit;
        if TrackingLine."Quantity (Base)" * SourceQuantityArray[1] < 0 then if SourceQuantityArray[1] < 0 then Error(Text002, Text003)
            else
    ...
    protected procedure UpdateUndefinedQty(): Boolean var
        IsHandled: Boolean;
        ReturnValue: Boolean;
    begin
        IsHandled:=false;
 ...
 exit(true);
    ...
    local procedure TempRecIsValid()OK: Boolean var
        ReservEntry: Record "Reservation Entry";
        RecordCount: Integer;
    ...
    begin
        ...
        if ReservEntry.FindSet()then repeat if not TempReservEntry.Get(ReservEntry."Entry No.", ReservEntry.Positive)then exit(false);
                if not EntriesAreIdentical(ReservEntry, TempReservEntry, IdenticalArray)then exit(false);
                RecordCount+=1;
            until ReservEntry.Next() = 0;
        OK:=RecordCount = TempReservEntry.Count();
    end;
    procedure EntriesAreIdentical(var ReservEntry1: Record "Reservation Entry"; var ReservEntry2: Record "Reservation Entry"; var IdenticalArray: array[2]of Boolean): Boolean begin
        IdenticalArray[1]:=(ReservEntry1."Entry No." = ReservEntry2."Entry No.") and (ReservEntry1."Item No." = ReservEntry2."Item No.") and (ReservEntry1."Location Code" = ReservEntry2."Location Code") and (ReservEntry1."Quantity (Base)" = ReservEntry2."Quantity (Base)") and (ReservEntry1."Reservation Status" = ReservEntry2."Reservation Status") and (ReservEntry1."Creation Date" = ReservEntry2."Creation Date") and (ReservEntry1."Transferred from Entry No." = ReservEntry2."Transferred from Entry No.") and (ReservEntry1."Source Type" = ReservEntry2."Source Type") and (ReservEntry1."Source Subtype" = ReservEntry2."Source Subtype") and (ReservEntry1."Source ID" = ReservEntry2."Source ID") and (ReservEntry1."Source Batch Name" = ReservEntry2."Source Batch Name") and (ReservEntry1."Source Prod. Order Line" = ReservEntry2."Source Prod. Order Line") and (ReservEntry1."Source Ref. No." = ReservEntry2."Source Ref. No.") and (ReservEntry1."Expected Receipt Date" = ReservEntry2."Expected Receipt Date") and (ReservEntry1."Shipment Date" = ReservEntry2."Shipment Date") and (ReservEntry1."Created By" = ReservEntry2."Created By") and (ReservEntry1."Changed By" = ReservEntry2."Changed By") and (ReservEntry1.Positive = ReservEntry2.Positive) and (ReservEntry1."Qty. per Unit of Measure" = ReservEntry2."Qty. per Unit of Measure") and (ReservEntry1.Quantity = ReservEntry2.Quantity) and (ReservEntry1."Action Message Adjustment" = ReservEntry2."Action Message Adjustment") and (ReservEntry1.Binding = ReservEntry2.Binding) and (ReservEntry1."Suppressed Action Msg." = ReservEntry2."Suppressed Action Msg.") and (ReservEntry1."Planning Flexibility" = ReservEntry2."Planning Flexibility") and (ReservEntry1."Variant Code" = ReservEntry2."Variant Code") and (ReservEntry1."Quantity Invoiced (Base)" = ReservEntry2."Quantity Invoiced (Base)") and ReservEntry1.HasSameTracking(ReservEntry2);
        IdenticalArray[2]:=(ReservEntry1.Description = ReservEntry2.Description) and (ReservEntry1."Expiration Date" = ReservEntry2."Expiration Date") and (ReservEntry1."Warranty Date" = ReservEntry2."Warranty Date") and (ReservEntry1."New Expiration Date" = ReservEntry2."New Expiration Date") and ReservEntry1.HasSameNewTracking(ReservEntry2);
        OnAfterEntriesAreIdentical(ReservEntry1, ReservEntry2, IdenticalArray);
        exit(IdenticalArray[1] and IdenticalArray[2]);
    end;
    local procedure QtyToHandleAndInvoiceChanged(var ReservEntry1: Record "Reservation Entry"; var ReservEntry2: Record "Reservation Entry")HasChanged: Boolean begin
        HasChanged:=(ReservEntry1."Qty. to Handle (Base)" <> ReservEntry2."Qty. to Handle (Base)") or (ReservEntry1."Qty. to Invoice (Base)" <> ReservEntry2."Qty. to Invoice (Base)");
        OnAfterQtyToHandleAndInvoiceChanged(ReservEntry1, ReservEntry2, HasChanged);
    end;
    procedure NextEntryNo(): Integer begin
        LastEntryNo+=1;
        exit(LastEntryNo);
    end;
    ...
    begin
        ...
        if CurrentPageIsOpen then begin
            ...
            TempRecValid();
            if Item."Order Tracking Policy" = Item."Order Tracking Policy"::None then QtyToAddAsBlank:=0
            else
            ...
            if TempItemTrackLineDelete.Find('-')then begin
                repeat i:=i + 1;
                    if i mod 100 = 0 then Window.Update(2, Round(i / NoOfLines * 10000, 1));
                    RegisterChange(TempItemTrackLineDelete, TempItemTrackLineDelete, ChangeType::Delete, false);
                    if TempItemTrackLineModify.Get(TempItemTrackLineDelete."Entry No.")then TempItemTrackLineModify.Delete();
                until TempItemTrackLineDelete.Next() = 0;
                TempItemTrackLineDelete.DeleteAll();
            end;
            for ModifyLoop:=1 to 2 do begin
                if TempItemTrackLineModify.Find('-')then repeat if TempTrackingSpecification2.Get(TempItemTrackLineModify."Entry No.")then begin
                            ...
                            if((ModifyLoop = 1) and Decrease) or ((ModifyLoop = 2) and not Decrease)then begin
                                ...
                                if ShouldModifyTrackingSpecification(TempTrackingSpecification2, TempItemTrackLineModify)then begin
                                    ...
                                    if QtyToHandleOrInvoiceDifferFromQuantity(TempItemTrackLineModify)then SetQtyToHandleAndInvoice(TempItemTrackLineModify);
                                end
                                else
                                begin
            ...
            if TempItemTrackLineInsert.Find('-')then begin
                repeat i:=i + 1;
                    if i mod 100 = 0 then Window.Update(2, Round(i / NoOfLines * 10000, 1));
                    if TempItemTrackLineModify.Get(TempItemTrackLineInsert."Entry No.")then TempItemTrackLineInsert.TransferFields(TempItemTrackLineModify);
                    OnWriteToDatabaseOnBeforeRegisterInsert(TempItemTrackLineInsert);
                    if not RegisterChange(TempItemTrackLineInsert, TempItemTrackLineInsert, ChangeType::Insert, false)then Error(Text005);
                    if QtyToHandleOrInvoiceDifferFromQuantity(TempItemTrackLineInsert)then SetQtyToHandleAndInvoice(TempItemTrackLineInsert);
        ...
        begin
            ...
            if TempTrackingSpecification2.Find('-')then repeat Rec.SetTrackingFilterFromSpec(TempTrackingSpecification2);
            ...
            Rec.Reset();
            if Rec.Find('-')then repeat if RegisterChange(Rec, Rec, ChangeType::Insert, false)then begin
                        TempTrackingSpecification2:=Rec;
                        TempTrackingSpecification2.Insert();
                    end
                    else
        ...
        if not BlockCommit then Commit();
    end;
    local procedure ShouldModifyTrackingSpecification(TrackingSpecification: Record "Tracking Specification"; TrackingSpecificationModify: Record "Tracking Specification"): Boolean var
        IsHandled: Boolean;
        Result: Boolean;
    begin
        IsHandled:=false;
        ...
        if IsHandled then exit(Result);
        exit((not TrackingSpecification.HasSameTracking(TrackingSpecificationModify)) or (TrackingSpecification."Appl.-from Item Entry" <> TrackingSpecificationModify."Appl.-from Item Entry") or (TrackingSpecification."Appl.-to Item Entry" <> TrackingSpecificationModify."Appl.-to Item Entry"));
    end;
    protected procedure RegisterChange(var OldTrackingSpecification: Record "Tracking Specification"; var NewTrackingSpecification: Record "Tracking Specification"; ChangeType: Option Insert, Modify, FullDelete, PartDelete, ModifyAll; ModifySharedFields: Boolean)OK: Boolean var
    ...
    begin
        ...
        if IsHandled then exit;
        OK:=false;
        if((CurrentSignFactor * NewTrackingSpecification."Qty. to Handle") < 0) and (CurrentRunMode <> CurrentRunMode::"Drop Shipment")then begin
            NewTrackingSpecification."Expiration Date":=0D;
            OldTrackingSpecification."Expiration Date":=0D;
        end;
        case ChangeType of ChangeType::Insert: begin
            IsHandled:=false;
            OnRegisterChangeOnBeforeInsert(NewTrackingSpecification, OldTrackingSpecification, IsHandled);
            if IsHandled then exit(true);
            if(OldTrackingSpecification."Quantity (Base)" = 0) or not OldTrackingSpecification.TrackingExists()then exit(true);
            TempReservEntry.SetTrackingFilterBlank();
            OldTrackingSpecification."Quantity (Base)":=CurrentSignFactor * ReservEngineMgt.AddItemTrackingToTempRecSet(TempReservEntry, NewTrackingSpecification, CurrentSignFactor * OldTrackingSpecification."Quantity (Base)", QtyToAddAsBlank, ItemTrackingCode);
            TempReservEntry.ClearTrackingFilter();
            // Late Binding
            ProcessLateBinding(NewTrackingSpecification);
            if OldTrackingSpecification."Quantity (Base)" = 0 then exit(true);
            ...
            OnRegisterChangeOnChangeTypeInsertOnBeforeInsertReservEntry(Rec, OldTrackingSpecification, NewTrackingSpecification, CurrentRunMode.AsInteger());
            CreateReservEntry.SetDates(NewTrackingSpecification."Warranty Date", NewTrackingSpecification."Expiration Date");
            ...
            CreateReservEntry.CreateReservEntryFor(OldTrackingSpecification."Source Type", OldTrackingSpecification."Source Subtype", OldTrackingSpecification."Source ID", OldTrackingSpecification."Source Batch Name", OldTrackingSpecification."Source Prod. Order Line", OldTrackingSpecification."Source Ref. No.", OldTrackingSpecification."Qty. per Unit of Measure", 0, OldTrackingSpecification."Quantity (Base)", ReservEntry1);
            ...
            CreateReservEntry.CreateEntry(OldTrackingSpecification."Item No.", OldTrackingSpecification."Variant Code", OldTrackingSpecification."Location Code", OldTrackingSpecification.Description, ExpectedReceiptDate, ShipmentDate, 0, CurrentEntryStatus);
            CreateReservEntry.GetLastEntry(ReservEntry1);
            OnRegisterChangeOnAfterCreateReservEntry(ReservEntry1, NewTrackingSpecification, OldTrackingSpecification, CurrentRunMode, CurrentSourceType, TempReservEntry);
            if Item."Order Tracking Policy" = Item."Order Tracking Policy"::"Tracking & Action Msg." then ReservEngineMgt.UpdateActionMessages(ReservEntry1);
        ...
        ChangeType::Modify: begin
            ...
            if EntriesAreIdentical(ReservEntry1, ReservEntry2, IdenticalArray)then exit(QtyToHandleAndInvoiceChanged(ReservEntry1, ReservEntry2));
            if ShouldAddQuantityAsBlank(OldTrackingSpecification, NewTrackingSpecification)then begin
                ...
                TempReservEntry.SetTrackingFilterBlank();
                OnRegisterChangeOnBeforeAddItemTrackingToTempRecSet(OldTrackingSpecification, NewTrackingSpecification, CurrentSignFactor, TempReservEntry);
                QtyToAdd:=CurrentSignFactor * ReservEngineMgt.AddItemTrackingToTempRecSet(TempReservEntry, NewTrackingSpecification, CurrentSignFactor * (NewTrackingSpecification."Quantity (Base)" - OldTrackingSpecification."Quantity (Base)"), QtyToAddAsBlank, ItemTrackingCode);
                TempReservEntry.ClearTrackingFilter();
                // Late Binding
                ProcessLateBinding(NewTrackingSpecification);
                ...
                RegisterChange(OldTrackingSpecification, OldTrackingSpecification, ChangeType::Insert, not IdenticalArray[2]);
            end
            else
            begin
                ...
                OnRegisterChangeOnBeforeAddItemTrackingToTempRecSet(OldTrackingSpecification, NewTrackingSpecification, CurrentSignFactor, TempReservEntry);
                QtyToAdd:=CurrentSignFactor * ReservEngineMgt.AddItemTrackingToTempRecSet(TempReservEntry, OldTrackingSpecification, CurrentSignFactor * (OldTrackingSpecification."Quantity (Base)" - NewTrackingSpecification."Quantity (Base)"), QtyToAddAsBlank, ItemTrackingCode);
                TempReservEntry.ClearTrackingFilter();
                RegisterChange(NewTrackingSpecification, NewTrackingSpecification, ChangeType::PartDelete, not IdenticalArray[2]);
            end;
        ...
        ChangeType::FullDelete, ChangeType::PartDelete: begin
            ...
            if ChangeType = ChangeType::FullDelete then begin
                ...
                OldTrackingSpecification.ClearTracking();
                OnAfterClearTrackingSpec(OldTrackingSpecification);
                QtyToAdd:=CurrentSignFactor * ReservEngineMgt.AddItemTrackingToTempRecSet(TempReservEntry, OldTrackingSpecification, CurrentSignFactor * OldTrackingSpecification."Quantity (Base)", QtyToAddAsBlank, ItemTrackingCode);
                TempReservEntry.ClearTrackingFilter();
            ...
            begin
                ReservationMgt.DeleteReservEntries(false, ReservEntry1."Quantity (Base)" - OldTrackingSpecification."Quantity Handled (Base)", ReservEntry1);
                if ModifySharedFields then begin
    ...
    local procedure ShouldAddQuantityAsBlank(OldTrackingSpecification: Record "Tracking Specification"; NewTrackingSpecification: Record "Tracking Specification"): Boolean var
        IsHandled: Boolean;
        Result: Boolean;
    begin
        IsHandled:=false;
        ...
        if IsHandled then exit(Result);
        exit(Abs(OldTrackingSpecification."Quantity (Base)") < Abs(NewTrackingSpecification."Quantity (Base)"));
    end;
    ...
    begin
        ...
        if ReservEngineMgt.CollectAffectedSurplusEntries(TempReservEntry)then begin
            ...
            if Item."Order Tracking Policy" <> Item."Order Tracking Policy"::None then ReservEngineMgt.UpdateOrderTracking(TempReservEntry);
        end;
        // Late Binding
        if TempItemTrackLineReserv.FindSet()then repeat LateBindingMgt.ReserveItemTrackingLine(TempItemTrackLineReserv, 0, TempItemTrackLineReserv."Quantity (Base)");
    ...
    begin
        ...
        if ReservEntry1.Find('-')then repeat ReservEntry1.Description:=TrackingSpecification.Description;
    ...
    begin
        ...
        if TrackingSpecification.TrackingExists()then begin
            ...
            if TrackingSpecification.Find('-')then repeat if not TrackingSpecification.Correction then begin
                        ModifyLine:=false;
                        QtyToInvoiceThisLine:=TrackingSpecification."Quantity Handled (Base)" - TrackingSpecification."Quantity Invoiced (Base)";
                        if Abs(QtyToInvoiceThisLine) > Abs(TotalQtyToInvoice)then QtyToInvoiceThisLine:=TotalQtyToInvoice;
                        ...
                        if ModifyLine then TrackingSpecification.Modify();
                        TotalQtyToInvoice-=QtyToInvoiceThisLine;
                    end;
                until(TrackingSpecification.Next() = 0);
        end;
        if TrackingSpecification.NonSerialTrackingExists()then begin
            if(TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and (TrackingSpecification."Source Subtype" = 1) and (TrackingSpecification."Source Prod. Order Line" <> 0) // Shipped
            then ReservEntry1.SetRange("Source Ref. No.");
            ...
            if ReservEntry1.FindSet()then repeat ModifyLine:=false;
                    QtyToHandleThisLine:=ReservEntry1."Quantity (Base)";
                    QtyToInvoiceThisLine:=QtyToHandleThisLine;
                    if Abs(QtyToHandleThisLine) > Abs(TotalQtyToHandle)then QtyToHandleThisLine:=TotalQtyToHandle;
                    if Abs(QtyToInvoiceThisLine) > Abs(TotalQtyToInvoice)then QtyToInvoiceThisLine:=TotalQtyToInvoice;
                    if(ReservEntry1."Qty. to Handle (Base)" <> QtyToHandleThisLine) or (ReservEntry1."Qty. to Invoice (Base)" <> QtyToInvoiceThisLine) and not ReservEntry1.Correction then begin
                    ...
                    if ModifyLine then ReservEntry1.Modify();
                    TotalQtyToHandle-=QtyToHandleThisLine;
                    TotalQtyToInvoice-=QtyToInvoiceThisLine;
                until ReservEntry1.Next() = 0 end
        else if ReservEntry1.Find('-')then begin
                if(ReservEntry1."Qty. to Handle (Base)" <> TotalQtyToHandle) or (ReservEntry1."Qty. to Invoice (Base)" <> TotalQtyToInvoice) and not ReservEntry1.Correction then begin
    ...
    begin
        ...
        if TrackingSpecification."Source Type" <> DATABASE::"Transfer Line" then exit;
        ...
        case TrackingSpecification."Source Subtype" of 0: // Outbound
 ...
 ItemEntryRelation.SetRange("Source Type", DATABASE::"Transfer Receipt Line");
        ...
        if ItemEntryRelation.Find('-')then repeat ItemLedgerEntry.Get(ItemEntryRelation."Item Entry No.");
    ...
    begin
        ...
        if(TrackingSpecification."Source Type" <> DATABASE::"Assembly Line") and (TrackingSpecification."Source Type" <> DATABASE::"Assembly Header")then exit;
        ...
        if CurrentQtyBase = MaxQtyBase then exit;
        ItemEntryRelation.SetCurrentKey("Order No.", "Order Line No.");
        ItemEntryRelation.SetRange("Order No.", TrackingSpecification."Source ID");
        ItemEntryRelation.SetRange("Order Line No.", TrackingSpecification."Source Ref. No.");
        if TrackingSpecification."Source Type" = DATABASE::"Assembly Line" then ItemEntryRelation.SetRange("Source Type", DATABASE::"Posted Assembly Line")
        else
        ...
        if ItemEntryRelation.Find('-')then repeat ItemLedgerEntry.Get(ItemEntryRelation."Item Entry No.");
                ...
                TempTrackingSpecification."Qty. per Unit of Measure":=ItemLedgerEntry."Qty. per Unit of Measure";
                TempTrackingSpecification.InitQtyToShip();
                if TempTrackingSpecification."Quantity (Base)" * CurrentSignFactor > Abs(MaxQtyBase - CurrentQtyBase)then TempTrackingSpecification."Quantity (Base)":=MaxQtyBase - CurrentQtyBase;
                CurrentQtyBase+=TempTrackingSpecification."Quantity (Base)";
                OnBeforeCollectTempTrackingSpecificationInsert(TempTrackingSpecification, ItemLedgerEntry, TrackingSpecification);
                TempTrackingSpecification.Insert();
            until ItemEntryRelation.Next() = 0;
    ...
    begin
        ...
        if TrackingSpecification."Source Type" <> DATABASE::"Prod. Order Line" then exit;
        ...
        if ItemLedgerEntry.Find('-')then begin
            repeat TempTrackingSpecification:=TrackingSpecification;
                TempTrackingSpecification."Entry No.":=ItemLedgerEntry."Entry No.";
                TempTrackingSpecification."Item No.":=ItemLedgerEntry."Item No.";
                TempTrackingSpecification.CopyTrackingFromItemLedgEntry(ItemLedgerEntry);
                TempTrackingSpecification."Quantity (Base)":=ItemLedgerEntry.Quantity;
            ...
            if ItemLedgerEntry.Quantity > SourceQuantityArray[1]then SourceQuantityArray[1]:=ItemLedgerEntry.Quantity;
        end;
        OnAfterCollectPostedOutputEntries(ItemLedgerEntry, TempTrackingSpecification);
    end;
    procedure ZeroLineExists()OK: Boolean var
        xTrackingSpec: Record "Tracking Specification";
    begin
        if(Rec."Quantity (Base)" <> 0) or Rec.TrackingExists()then exit(false);
        xTrackingSpec.Copy(Rec);
        Rec.Reset();
    ...
    begin
        if ZeroLineExists()then Rec.Delete();
        QtyToCreate:=UndefinedQtyArray[1] * QtySignFactor();
        if QtyToCreate < 0 then QtyToCreate:=0;
        if QtyToCreate mod 1 <> 0 then Error(Text008);
    ...
    begin
        if QtyToCreate <= 0 then Error(Text009);
        if QtyToCreate mod 1 <> 0 then Error(Text008);
        GetItem(Rec."Item No.");
        ...
        for i:=1 to QtyToCreate do begin
            ...
            Rec."Entry No.":=NextEntryNo();
            if TestTempSpecificationExists(CheckTillEntryNo)then Error('');
            Rec.Insert();
            OnAssignSerialNoBatchOnAfterInsert(Rec, QtyToCreate, CreateLotNo);
            TempItemTrackLineInsert.TransferFields(Rec);
            TempItemTrackLineInsert.Insert();
            if i = QtyToCreate then ItemTrackingDataCollection.SetSkipLot(false);
            ItemTrackingDataCollection.UpdateTrackingDataSetWithChange(TempItemTrackLineInsert, CurrentSignFactor * SourceQuantityArray[1] < 0, CurrentSignFactor, 0);
            if CreateSNInfo then ItemTrackingMgt.CreateSerialNoInformation(Rec);
        end;
        CalculateSums();
    end;
    ...
    begin
        ...
        if ZeroLineExists()then Rec.Delete();
        if(SourceQuantityArray[1] * UndefinedQtyArray[1] <= 0) or (Abs(SourceQuantityArray[1]) < Abs(UndefinedQtyArray[1]))then QtyToCreate:=0
        else
    ...
    var
        ...
        CheckTillEntryNo: Integer;
    begin
        if IncStr(CustomizedSN) = '' then Error(StrSubstNo(UnincrementableStringErr, CustomizedSN));
        ...
        for i:=1 to QtyToCreate do begin
            ...
            if i < QtyToCreate then begin
                Counter:=Increment;
                repeat CustomizedSN:=IncStr(CustomizedSN);
                    Counter:=Counter - 1;
                until Counter <= 0;
            end;
        end;
    ...
    procedure TestTempSpecificationExists()Exists: Boolean begin
        TestTempSpecificationExists(-1);
    end;
    local procedure TestTempSpecificationExists(CheckTillEntryNo: Integer)Exists: Boolean var
        TrackingSpecification: Record "Tracking Specification";
    begin
        if not Rec.TrackingExists()then exit(false);
        TrackingSpecification.Copy(Rec);
        Rec.SetTrackingKey();
        Rec.SetRange("Serial No.", Rec."Serial No.");
        if Rec."Serial No." = '' then Rec.SetNonSerialTrackingFilterFromSpec(Rec);
        if CheckTillEntryNo = -1 then Rec.SetFilter("Entry No.", '<>%1', Rec."Entry No.")
        else
        ...
        if Exists and CurrentPageIsOpen then if Rec."Serial No." = '' then Message(Text011, Rec."Serial No.", Rec."Lot No.", Rec."Package No.")
            else
                Message(Text012, Rec."Serial No.");
    end;
    protected procedure QtySignFactor(): Integer begin
        if SourceQuantityArray[1] < 0 then exit(-1);
        exit(1)end;
    ...
    begin
        ...
        if not CalledFromSynchWhseItemTrkg then TempTrackingSpecification.Reset();
        if not TempTrackingSpecification.Find('-')then exit;
        ...
        Rec.Reset();
        Rec.SetTrackingKey();
        repeat Rec.SetTrackingFilterFromSpec(TempTrackingSpecification);
            ...
            if Rec.Find('-')then begin
                ...
                begin
                    ...
                    if not IsHandled then Rec.Validate("Quantity (Base)", Rec."Quantity (Base)" + TempTrackingSpecification."Quantity (Base)");
                end;
        ...
        Rec.Reset();
        if Rec.Find('-')then repeat CheckItemTrackingLine(Rec);
            until Rec.Next() = 0;
        Rec.SetTrackingFilterFromSpec(SourceTrackingSpecification);
        CalculateSums();
        if UpdateUndefinedQty()then WriteToDatabase()
        else
            Error(Text014, TotalTrackingSpecification."Quantity (Base)", LowerCase(TempReservEntry.TextCaption()), SourceQuantityArray[1]);
        // Copy to inbound part of transfer
        if(CurrentRunMode = CurrentRunMode::Transfer) or IsOrderToOrderBindingToTransfer()then SynchronizeLinkedSources('');
    end;
    procedure SynchronizeLinkedSources(DialogText: Text[250]): Boolean begin
        ...
        if CurrentSourceRowID = '' then exit(false);
        if SecondSourceRowID = '' then exit(false);
    ...
    local procedure IsExpirationDateEditable(): Boolean begin
        if(Rec."Buffer Status2" = Rec."Buffer Status2"::"ExpDate blocked") or not ItemTrackingCode."Use Expiration Dates" then exit(false);
        if InboundIsSet then exit(Inbound);
    ...
    local procedure TrackingAvailable(var TrackingSpecification: Record "Tracking Specification"; LookupMode: Enum "Item Tracking Type"): Boolean begin
    ...
    begin
        ...
        MaxQuantity:=UndefinedQtyArray[1];
        if MaxQuantity * CurrentSignFactor > 0 then MaxQuantity:=0;
        Rec."Bin Code":=ForBinCode;
        ...
        Rec."Bin Code":='';
        if Rec.FindSet()then repeat case Rec."Buffer Status" of Rec."Buffer Status"::MODIFY: begin
                    if TempItemTrackLineModify.Get(Rec."Entry No.")then TempItemTrackLineModify.Delete();
                    if TempItemTrackLineInsert.Get(Rec."Entry No.")then begin
                        TempItemTrackLineInsert.TransferFields(Rec);
                ...
                Rec."Buffer Status"::INSERT: begin
                    TempItemTrackLineInsert.TransferFields(Rec);
                    OnSelectEntriesOnAfterTransferFields(TempItemTrackLineInsert, Rec);
                    TempItemTrackLineInsert.Insert();
                end;
    ...
    begin
        if not TempRecIsValid()then Error(Text007);
    end;
    ...
    begin
        ...
        if TempItemTrackLineInsert.Find('-')then repeat TempTrackingSpecInsert2.Init();
        ...
        if TempItemTrackLineModify.Find('-')then repeat TempTrackingSpecModify2.Init();
        ...
        if TempItemTrackLineDelete.Find('-')then repeat TempTrackingSpecDelete2.Init();
    ...
    begin
        ...
        TempItemTrackLineInsert.Reset();
        TempTrackingSpecInsert2.Reset();
        if TempTrackingSpecInsert2.Find('-')then repeat TempItemTrackLineInsert.Init();
        ...
        TempItemTrackLineModify.Reset();
        TempTrackingSpecModify2.Reset();
        if TempTrackingSpecModify2.Find('-')then repeat TempItemTrackLineModify.Init();
        ...
        TempItemTrackLineDelete.Reset();
        TempTrackingSpecDelete2.Reset();
        if TempTrackingSpecDelete2.Find('-')then repeat TempItemTrackLineDelete.Init();
                TempItemTrackLineDelete:=TempTrackingSpecDelete2;
                TempItemTrackLineDelete.Insert();
            until TempTrackingSpecDelete2.Next() = 0;
    end;
    local procedure GetHandleSource(TrackingSpecification: Record "Tracking Specification"): Boolean var
    ...
    begin
        with TrackingSpecification do begin
            ...
            QtyToHandleColumnIsHidden:=("Source Type" in[DATABASE::"Item Ledger Entry", DATABASE::"Item Journal Line", DATABASE::"Job Journal Line", DATABASE::"Requisition Line"]) or (("Source Type" in[DATABASE::"Sales Line", DATABASE::"Purchase Line", DATABASE::"Service Line"]) and ("Source Subtype" in[0, 2, 3])) or (("Source Type" = DATABASE::"Assembly Line") and ("Source Subtype" = 0));
        end;
        OnAfterGetHandleSource(TrackingSpecification, QtyToHandleColumnIsHidden);
        exit(not QtyToHandleColumnIsHidden);
    end;
    local procedure GetInvoiceSource(TrackingSpecification: Record "Tracking Specification"): Boolean var
        QtyToInvoiceColumnIsHidden: Boolean;
    begin
        with TrackingSpecification do begin
            QtyToInvoiceColumnIsHidden:=("Source Type" in[DATABASE::"Item Ledger Entry", DATABASE::"Item Journal Line", DATABASE::"Job Journal Line", DATABASE::"Requisition Line", DATABASE::"Transfer Line", DATABASE::"Assembly Line", DATABASE::"Assembly Header", DATABASE::"Prod. Order Line", DATABASE::"Prod. Order Component"]) or (("Source Type" in[DATABASE::"Sales Line", DATABASE::"Purchase Line", DATABASE::"Service Line"]) and ("Source Subtype" in[0, 2, 3, 4]))end;
    ...
    begin
        TempTrackingSpecification.DeleteAll();
        if Rec.FindSet()then repeat TempTrackingSpecification:=Rec;
                TempTrackingSpecification.Insert();
            until Rec.Next() = 0;
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        if ItemTrackingMgt.ItemTrkgIsManagedByWhse(Rec."Source Type", Rec."Source Subtype", Rec."Source ID", Rec."Source Prod. Order Line", Rec."Source Ref. No.", Rec."Location Code", Rec."Item No.")then exit;
        WhseManagement.SetSourceFilterForWhseShptLine(WarehouseShipmentLine, Rec."Source Type", Rec."Source Subtype", Rec."Source ID", Rec."Source Ref. No.", true);
        if WarehouseShipmentLine.IsEmpty()then exit;
        ...
        if not(Location.RequirePicking(Rec."Location Code") and Location.RequirePutaway(Rec."Location Code"))then begin
            ...
            WarehouseEntry.SetFilter("Reference Document", '%1|%2', WarehouseEntry."Reference Document"::"Put-away", WarehouseEntry."Reference Document"::Pick);
            if not WarehouseEntry.IsEmpty()then exit;
        end;
        repeat WarehouseShipmentLine.DeleteWhseItemTrackingLines();
            WarehouseShipmentLine.CreateWhseItemTrackingLines();
        until WarehouseShipmentLine.Next() = 0;
    end;
    protected procedure IsOrderToOrderBindingToTransfer(): Boolean var
        ReservEntry: Record "Reservation Entry";
    begin
        if CurrentSourceType = DATABASE::"Transfer Line" then exit(false);
        ...
        if not ReservEntry.FindFirst()then exit(false);
        ReservEntry.Get(ReservEntry."Entry No.", not ReservEntry.Positive);
        if not((ReservEntry."Source Type" = DATABASE::"Transfer Line") and (ReservEntry."Source Subtype" = 0))then exit(false);
        CurrentSourceRowID:=ItemTrackingMgt.ComposeRowID(ReservEntry."Source Type", 0, ReservEntry."Source ID", ReservEntry."Source Batch Name", ReservEntry."Source Prod. Order Line", ReservEntry."Source Ref. No.");
        SecondSourceRowID:=ItemTrackingMgt.ComposeRowID(ReservEntry."Source Type", 1, ReservEntry."Source ID", ReservEntry."Source Batch Name", ReservEntry."Source Prod. Order Line", ReservEntry."Source Ref. No.");
        exit(true);
    end;
    ...
    begin
        ...
        if FindSet()then repeat if TempItemTrackLineModify.Get("Entry No.")then TempItemTrackLineModify.Delete();
    ...
    local procedure QtyToHandleOrInvoiceDifferFromQuantity(ItemTrackingLine: Record "Tracking Specification")HasChanged: Boolean begin
        HasChanged:=(ItemTrackingLine."Quantity (Base)" <> ItemTrackingLine."Qty. to Handle (Base)") or (ItemTrackingLine."Quantity (Base)" <> ItemTrackingLine."Qty. to Invoice (Base)");
...
#if not CLEAN20[Obsolete('Replaced by OnAfterSetPageControls()', '20.0')]
...
#endif[IntegrationEvent(TRUE, false)]

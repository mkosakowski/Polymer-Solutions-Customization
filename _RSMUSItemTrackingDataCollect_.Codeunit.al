codeunit 50108 "RSMUSItemTrackingDataCollect"
{
    ...
    var TempGlobalReservEntry: Record "Reservation Entry" temporary;
    ...
    begin
        ...
        if not FullGlobalDataSetExists then RetrieveLookupData(TempTrackingSpecification, true);
        ...
        if TempGlobalEntrySummary.IsEmpty()then TempGlobalEntrySummary.SetTrackingKey();
        ...
        case LookupMode of LookupMode::"Serial No.": AssistEditTrackingNoLookupSerialNo(TempTrackingSpecification, ItemTrackingSummaryForm);
        LookupMode::"Lot No.": AssistEditTrackingNoLookupLotNo(TempTrackingSpecification, ItemTrackingSummaryForm);
        else
            OnAssistEditTrackingNoOnLookupModeElseCase(TempTrackingSpecification, ItemTrackingSummaryForm, TempGlobalEntrySummary, LookupMode, LookupMode);
        end;
        ...
        if ItemTrackingSummaryForm.RunModal() = ACTION::LookupOK then begin
            ...
            if TempGlobalEntrySummary."Bin Active" then AvailableQty:=MinValueAbs(TempGlobalEntrySummary."Bin Content", TempGlobalEntrySummary."Total Available Quantity")
            else
            ...
            if MaxQuantity < 0 then begin
                ...
                if AvailableQty < 0 then if AdjustmentQty > AvailableQty then AdjustmentQty:=AvailableQty;
                if QtyOnLine + AdjustmentQty < 0 then AdjustmentQty:=-QtyOnLine;
            end
            else
            begin
            ...
            if LookupMode = LookupMode::"Serial No." then TempTrackingSpecification.Validate("Serial No.", TempGlobalEntrySummary."Serial No.");
            ...
            if TempTrackingSpecification.IsReclass() or DirectTransfer then TempTrackingSpecification.CopyNewTrackingFromTrackingSpec(TempTrackingSpecification);
    ...
    local procedure CalcNewQtyOnLine(var TempTrackingSpecification: Record "Tracking Specification" temporary; QtyOnLine: Decimal; AdjustmentQty: Decimal; QtyHandledOnLine: Decimal)NewQtyOnLine: Decimal var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        OnBeforeCalcNewQtyOnLine(TempTrackingSpecification, QtyOnLine, AdjustmentQty, QtyHandledOnLine, NewQtyOnLine, IsHandled);
        if IsHandled then exit(NewQtyOnLine);
        NewQtyOnLine:=QtyOnLine + AdjustmentQty + QtyHandledOnLine;
        if TempTrackingSpecification."Serial No." <> '' then if Abs(NewQtyOnLine) > 1 then NewQtyOnLine:=NewQtyOnLine / Abs(NewQtyOnLine); // Set to a signed value of 1.
    end;
    ...
    begin
        if TempTrackingSpecification."Lot No." <> '' then TempGlobalEntrySummary.SetRange("Lot No.", TempTrackingSpecification."Lot No.");
        ...
        if TempGlobalEntrySummary.FindFirst()then ItemTrackingSummaryPage.SetRecord(TempGlobalEntrySummary);
    ...
    begin
        if TempTrackingSpecification."Serial No." <> '' then TempGlobalEntrySummary.SetRange("Serial No.", TempTrackingSpecification."Serial No.")
        else
    ...
    begin
        ...
        if CurrentSignFactor > 0 then MaxQuantity:=-MaxQuantity;
        ...
        Window.Close();
        if not HideValidationDialog then if not(ItemTrackingSummaryForm.RunModal() = ACTION::LookupOK)then exit;
        ItemTrackingSummaryForm.GetSelected(TempEntrySummary);
        if TempEntrySummary.IsEmpty()then exit;
        // Swap sign on the selected entries if parent is a negative supply line
        if CurrentSignFactor > 0 then // Negative supply lines
 if TempEntrySummary.Find('-')then repeat TempEntrySummary."Selected Quantity":=-TempEntrySummary."Selected Quantity";
    ...
    var
        ...
        Window: Dialog;
    begin
        if ShouldExitLookupTrackingAvailability(TempTrackingSpecification, LookupMode)then exit;
        Clear(ItemTrackingSummaryForm);
        Window.Open(Text004);
        TempGlobalChangedEntrySummary.Reset();
        if not(PartialGlobalDataSetExists or FullGlobalDataSetExists)then RetrieveLookupData(TempTrackingSpecification, true);
        TempGlobalEntrySummary.Reset();
        TempGlobalEntrySummary.SetTrackingKey();
        TempGlobalReservEntry.Reset();
        case LookupMode of LookupMode::"Serial No.": begin
            TempGlobalEntrySummary.SetRange("Serial No.", TempTrackingSpecification."Serial No.");
            TempGlobalEntrySummary.SetFilter("Table ID", '<>%1', 0); // Filter out summations
            TempGlobalReservEntry.SetRange("Serial No.", TempTrackingSpecification."Serial No.");
            ItemTrackingSummaryForm.Caption:=StrSubstNo(AvailabilityText, TempTrackingSpecification.FieldCaption("Serial No."), TempTrackingSpecification."Serial No.");
        end;
        LookupMode::"Lot No.": begin
            TempGlobalEntrySummary.SetRange("Serial No.", '');
            TempGlobalEntrySummary.SetRange("Lot No.", TempTrackingSpecification."Lot No.");
            TempGlobalReservEntry.SetRange("Lot No.", TempTrackingSpecification."Lot No.");
            ItemTrackingSummaryForm.Caption:=StrSubstNo(AvailabilityText, TempTrackingSpecification.FieldCaption("Lot No."), TempTrackingSpecification."Lot No.");
        end;
        else
            OnLookupTrackingAvailabilityOnSetFiltersElseCase(TempGlobalEntrySummary, TempGlobalReservEntry, TempTrackingSpecification, ItemTrackingSummaryForm, LookupMode);
        end;
        ItemTrackingSummaryForm.SetSources(TempGlobalReservEntry, TempGlobalEntrySummary);
        ItemTrackingSummaryForm.SetCurrentBinAndItemTrkgCode(CurrBinCode, CurrItemTrackingCode);
    ...
    local procedure ShouldExitLookupTrackingAvailability(TempTrackingSpecification: Record "Tracking Specification" temporary; LookupMode: Enum "Item Tracking Type"): Boolean var
        ShouldExit: Boolean;
    begin
        case LookupMode of LookupMode::"Serial No.": if TempTrackingSpecification."Serial No." = '' then exit(true);
        LookupMode::"Lot No.": if TempTrackingSpecification."Lot No." = '' then exit(true);
        end;
        OnAfterShouldExitLookupTrackingAvailability(TempTrackingSpecification, LookupMode, ShouldExit);
    end;
    procedure RetrieveLookupData(var TempTrackingSpecification: Record "Tracking Specification" temporary; FullDataSet: Boolean): Code[50]var
    ...
    begin
        ...
        if ReservEntry.FindSet()then repeat TempReservEntry:=ReservEntry;
                if CanIncludeReservEntryToTrackingSpec(TempReservEntry)then TempReservEntry.Insert();
        ...
        else if TempTrackingSpecification.FindSet()then repeat if gLotNo = '' then gLotNo:=TempTrackingSpecification."Lot No.";
                    if gLotNo <> '' then exit(gLotNo);
    ...
    var
        IsHandled: Boolean;
    begin
        if ItemLedgEntry.FindSet()then repeat if ItemLedgEntry.TrackingExists() and not TempGlobalReservEntry.Get(-ItemLedgEntry."Entry No.", ItemLedgEntry.Positive)then begin
                    ...
                    if TempGlobalReservEntry.Positive then begin
                        ...
                        TempGlobalReservEntry."Expected Receipt Date":=0D end
                    else
    ...
    var
        IsHandled: Boolean;
    begin
        if TempReservEntry.FindSet()then repeat if not TempGlobalReservEntry.Get(TempReservEntry."Entry No.", TempReservEntry.Positive)then begin
    ...
    begin
        ...
        if SerialNoLookup then begin
            if TempReservEntry."Serial No." = '' then exit;
            TempGlobalEntrySummary.SetTrackingFilterFromReservEntry(TempReservEntry);
        end
        else
        begin
            if not TempReservEntry.NonSerialTrackingExists()then exit;
            TempGlobalEntrySummary.SetRange("Serial No.", '');
            TempGlobalEntrySummary.SetNonSerialTrackingFilterFromReservEntry(TempReservEntry);
            if TempReservEntry."Serial No." <> '' then TempGlobalEntrySummary.SetRange("Table ID", 0)
            else
                TempGlobalEntrySummary.SetFilter("Table ID", '<>%1', 0);
        end;
        ...
        if not TempGlobalEntrySummary.FindFirst()then begin
            ...
            if not SerialNoLookup and (TempReservEntry."Serial No." <> '')then TempGlobalEntrySummary."Table ID":=0 // Mark as summation
            else
                TempGlobalEntrySummary."Table ID":=TempReservEntry."Source Type";
            if SerialNoLookup then TempGlobalEntrySummary."Serial No.":=TempReservEntry."Serial No."
            else
            ...
            TempGlobalEntrySummary."Double-entry Adjustment":=MaxDoubleEntryAdjustQty(TempTrackingSpecification, TempGlobalEntrySummary);
        ...
        if TempReservEntry.Positive then begin
            ...
            if TempReservEntry."Entry No." < 0 then begin // The record represents an Item ledger entry
                TempGlobalEntrySummary."Non-specific Reserved Qty."+=LateBindingManagement.NonSpecificReservedQtyExceptForSource(-TempReservEntry."Entry No.", TempTrackingSpecification);
                TempGlobalEntrySummary."Total Quantity"+=TempReservEntry."Quantity (Base)";
            end;
            if TempReservEntry."Reservation Status" = TempReservEntry."Reservation Status"::Reservation then TempGlobalEntrySummary."Total Reserved Quantity"+=TempReservEntry."Quantity (Base)";
        end
        else
        begin
            ...
            if TempReservEntry.HasSamePointerWithSpec(TempTrackingSpecification)then begin
                if TempReservEntry."Reservation Status" = TempReservEntry."Reservation Status"::Reservation then TempGlobalEntrySummary."Current Reserved Quantity"-=TempReservEntry."Quantity (Base)";
 ...
 TempGlobalEntrySummary."Current Requested Quantity"-=TempReservEntry."Quantity (Base)";
        ...
        if DoInsert then TempGlobalEntrySummary.Insert()
        else
            TempGlobalEntrySummary.Modify();
        OnAfterCreateEntrySummary2(TempGlobalEntrySummary, TempReservEntry);
    end;
    local procedure MinValueAbs(Value1: Decimal; Value2: Decimal): Decimal begin
        if Abs(Value1) < Abs(Value2)then exit(Value1);
        exit(Value2);
    end;
    ...
    begin
        ...
        repeat TempTrackingSpecification.SetTrackingFilterFromEntrySummary(TempEntrySummary);
            if TempTrackingSpecification.FindFirst()then begin
                ...
                TempTrackingSpecification.Validate("Quantity (Base)", TempTrackingSpecification."Quantity (Base)" + TempEntrySummary."Selected Quantity");
            ...
            begin
                ...
                TransferExpDateFromSummary(TempTrackingSpecification, TempEntrySummary);
                if TempTrackingSpecification.IsReclass()then TempTrackingSpecification.CopyNewTrackingFromTrackingSpec(TempTrackingSpecification);
    ...
    procedure TrackingAvailable(TempTrackingSpecification: Record "Tracking Specification" temporary; LookupMode: Enum "Item Tracking Type"): Boolean var
    ...
    begin
        ...
        case LookupMode of LookupMode::"Serial No.": if(TempTrackingSpecification."Serial No." = '') or (not CurrItemTrackingCode."SN Specific Tracking")then exit(true);
        LookupMode::"Lot No.": if(TempTrackingSpecification."Lot No." = '') or (not CurrItemTrackingCode."Lot Specific Tracking")then exit(true);
        else
        begin
            IsHandled:=false;
        ...
        if CheckJobInPurchLine(TempTrackingSpecification)then exit(TempGlobalEntrySummary.FindFirst());
        exit(TempGlobalEntrySummary."Total Available Quantity" >= 0);
    end;
    procedure CheckAvailableTrackingQuantity(var TempTrackingSpecification: Record "Tracking Specification" temporary): Boolean var
        ItemTrackingSetup: Record "Item Tracking Setup";
    begin
        ...
        TempGlobalEntrySummary.SetTrackingKey();
        TempGlobalEntrySummary.SetTrackingFilterFromItemTrackingSetupIfRequired(ItemTrackingSetup);
        if TempGlobalEntrySummary.IsEmpty()then exit(false);
        TempGlobalEntrySummary.CalcSums("Total Available Quantity");
        exit(TempGlobalEntrySummary."Total Available Quantity" >= 0);
    end;
    ...
    var
        LastEntryNo: Integer;
    begin
        if not TempTrackingSpecificationChanged.TrackingExists()then exit;
    ...
    local procedure UpdateTrackingGlobalChangeRec(var TempTrackingSpecificationChanged: Record "Tracking Specification" temporary; LineIsDemand: Boolean; CurrentSignFactor: Integer; ChangeType: Option Insert, Modify, Delete): Integer var
        NewQuantity: Decimal;
        LastEntryNo: Integer;
    begin
        if(ChangeType = ChangeType::Delete) or not LineIsDemand then NewQuantity:=0
        else
 ...
 NewQuantity:=-NewQuantity;
        ...
        if not TempGlobalChangedEntrySummary.FindFirst()then begin
            ...
            if TempTrackingSpecificationChanged."Serial No." <> '' then TempGlobalChangedEntrySummary."Table ID":=DATABASE::"Tracking Specification"; // Not a summary line
            OnBeforeTempGlobalChangedEntrySummaryInsert(TempGlobalChangedEntrySummary, TempTrackingSpecificationChanged, LineIsDemand, CurrentSignFactor, ChangeType);
            TempGlobalChangedEntrySummary.Insert();
            PartialGlobalDataSetExists:=false; // The partial data set does not cover the new line
        end
        else if LineIsDemand then begin
                TempGlobalChangedEntrySummary."Current Pending Quantity":=NewQuantity;
                OnBeforeTempGlobalChangedEntrySummaryModify(TempGlobalChangedEntrySummary, TempTrackingSpecificationChanged, LineIsDemand, CurrentSignFactor, ChangeType);
    ...
    begin
        TempGlobalChangedEntrySummary.Reset();
        TempGlobalChangedEntrySummary.SetTrackingKey();
        if TempGlobalChangedEntrySummary.FindSet()then repeat if TempGlobalChangedEntrySummary.HasNonSerialTracking()then begin
                    // only last record with Lot Number updates Summary
                    if not TempGlobalChangedEntrySummary.HasSameNonSerialTracking(TempLastGlobalEntrySummary)then FindLastGlobalEntrySummary(TempGlobalChangedEntrySummary, TempLastGlobalEntrySummary);
    ...
    begin
        ...
        if TempChangedEntrySummary."Serial No." <> '' then begin
            ...
            if TempGlobalEntrySummary.FindFirst()then begin
                TempGlobalEntrySummary."Current Pending Quantity":=TempChangedEntrySummary."Current Pending Quantity" - TempGlobalEntrySummary."Current Requested Quantity";
                TempGlobalEntrySummary.UpdateAvailable();
                TempGlobalEntrySummary.Modify();
            end
            else
        ...
        if TempChangedEntrySummary.HasNonSerialTracking() and not SkipLot then begin
            TempGlobalEntrySummary.SetRange("Serial No.", '');
            TempGlobalEntrySummary.SetNonSerialTrackingFilterFromEntrySummary(TempChangedEntrySummary);
            if TempChangedEntrySummary."Serial No." <> '' then TempGlobalEntrySummary.SetRange("Table ID", 0)
            else
                TempGlobalEntrySummary.SetFilter("Table ID", '<>%1', 0);
            if TempGlobalEntrySummary.FindFirst()then begin
                if TempChangedEntrySummary."Serial No." <> '' then begin
 ...
 TempGlobalEntrySummary."Table ID":=0
    ...
    internal procedure GetFullGlobalDataSetExists(): Boolean begin
        exit(FullGlobalDataSetExists);
    end;
    procedure RefreshTrackingAvailability(var TempTrackingSpecification: Record "Tracking Specification" temporary; ShowMessage: Boolean)AvailabilityOK: Boolean begin
        AvailabilityOK:=RefreshTrackingAvailability(TempTrackingSpecification, true, ShowMessage);
    end;
    internal procedure RefreshTrackingAvailability(var TempTrackingSpecification: Record "Tracking Specification" temporary; RecreateGlobalDataSet: Boolean; ShowMessage: Boolean)AvailabilityOK: Boolean var
        TrackingSpecification2: Record "Tracking Specification";
        LookupMode: Enum "Item Tracking Type";
        PreviousLotNo: Code[50];
    begin
        AvailabilityOK:=true;
        if TempTrackingSpecification.Positive then exit;
        ...
        PreviousLotNo:='';
        repeat if TempTrackingSpecification."Lot No." <> PreviousLotNo then begin
                PreviousLotNo:=TempTrackingSpecification."Lot No.";
                LookupMode:=LookupMode::"Lot No.";
                if not TrackingAvailable(TempTrackingSpecification, LookupMode)then AvailabilityOK:=false;
                LookupMode:=LookupMode::"Serial No.";
            end;
            if not TrackingAvailable(TempTrackingSpecification, LookupMode)then AvailabilityOK:=false;
        until TempTrackingSpecification.Next() = 0;
        if ShowMessage then if AvailabilityOK then Message(NoAvailabilityWarningsMsg)
            else
    ...
    begin
        ...
        if xBinCode <> BinCode then if PartialGlobalDataSetExists then RefreshBinContent(TempGlobalEntrySummary);
    end;
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        if CurrBinCode = '' then exit;
        CurrItemTrackingCode.TestField(Code);
        WarehouseEntry.Reset();
        WarehouseEntry.SetCurrentKey("Item No.", "Bin Code", "Location Code", "Variant Code", "Unit of Measure Code", "Lot No.", "Serial No.", "Package No.");
    ...
    begin
        TempEntrySummary.Reset();
        if TempEntrySummary.FindSet()then repeat if CurrBinCode <> '' then UpdateBinContent(TempEntrySummary)
                else
    ...
    begin
        ...
        if TempEntrySummary."Total Quantity" <> 0 then begin
            ...
            if TrackingSpecification.IsReclass()then TrackingSpecification."New Expiration Date":=TrackingSpecification."Expiration Date"
            else
 ...
 exit;
        ...
        if TempGlobalReservEntry.FindSet()then repeat // Sum up per prod. order line per lot/sn
        ...
        TempGlobalReservEntry.SetRange("Source Subtype", 3); // Released order
        if TempGlobalReservEntry.FindSet()then repeat // Sum up per prod. order component per lot/sn
                SumUpTempTrkgSpec(TempGlobalTrackingSpec, TempGlobalReservEntry);
            until TempGlobalReservEntry.Next() = 0;
        TempGlobalReservEntry.Reset();
        TempGlobalReservEntry.SetCurrentKey("Source ID", "Source Ref. No.", "Source Type", "Source Subtype", "Source Batch Name");
        ...
        TempGlobalReservEntry.SetRange("Source Type", DATABASE::"Item Journal Line");
        TempGlobalReservEntry.SetRange("Source Subtype", 5, 6); // Consumption, Output
        if TempGlobalReservEntry.FindSet()then repeat // Sum up per Component line per lot/sn
 ...
 exit;
        ...
        if TempGlobalReservEntry.FindSet()then repeat // Sum up per job planning line per lot/sn
                SumUpTempTrkgSpec(TempGlobalTrackingSpec, TempGlobalReservEntry);
            until TempGlobalReservEntry.Next() = 0;
        TempGlobalReservEntry.Reset();
        TempGlobalReservEntry.SetCurrentKey("Source ID", "Source Ref. No.", "Source Type", "Source Subtype", "Source Batch Name");
        ...
        if TempGlobalReservEntry.FindSet()then repeat // Sum up per qty. line per lot/sn
    ...
    begin
        TempTrackingSpecification.SetSourceFilter(ReservEntry."Source Type", ReservEntry."Source Subtype", ReservEntry."Source ID", ReservEntry."Source Ref. No.", false);
 ...
 TempTrackingSpecification."Entry No."*=-1; // are handled.
    ...
    begin
        ...
        if not(ReservEntry."Source Subtype" in[5, 6])then ReservEntry.FieldError("Source Subtype");
        if not ItemJnlLine.Get(ReservEntry."Source ID", ReservEntry."Source Batch Name", ReservEntry."Source Ref. No.")then exit;
        if(ItemJnlLine."Order Type" <> ItemJnlLine."Order Type"::Production) or (ItemJnlLine."Order No." = '') or (ItemJnlLine."Order Line No." = 0)then exit;
        ...
        if FindRelatedParentTrkgSpec(ItemJnlLine, TempTrackingSpecification, ItemTrackingSetup)then begin
            ...
            ReservEntry.Modify();
            if(RemainingQty <> 0) and (RemainingQty * QtyOnJnlLine > 0)then if Abs(QtyOnJnlLine) <= Abs(RemainingQty)then AdjustQty:=-QtyOnJnlLine
                else
    ...
    local procedure FindRelatedParentTrkgSpec(ItemJnlLine: Record "Item Journal Line"; var TempTrackingSpecification: Record "Tracking Specification" temporary; ItemTrackingSetup: Record "Item Tracking Setup"): Boolean begin
        ItemJnlLine.TestField("Order Type", ItemJnlLine."Order Type"::Production);
        TempTrackingSpecification.Reset();
        case ItemJnlLine."Entry Type" of ItemJnlLine."Entry Type"::Consumption: begin
            if ItemJnlLine."Prod. Order Comp. Line No." = 0 then exit;
        ...
        ItemJnlLine."Entry Type"::Output: begin
    ...
    begin
        ...
        if not(ReservEntry."Source Subtype" = 0)then ReservEntry.FieldError("Source Subtype");
        if not JobJnlLine.Get(ReservEntry."Source ID", ReservEntry."Source Batch Name", ReservEntry."Source Ref. No.")then exit;
        // Buffer fields are used as follows:
        // "Buffer Value1" : Summed up quantity on journal line(s)
        // "Buffer Value2" : Adjustment needed to neutralize double entries
        ItemTrackingSetup.CopyTrackingFromReservEntry(ReservEntry);
    ...
    local procedure FindRelatedJobParentTrkgSpec(JobJnlLine: Record "Job Journal Line"; var TempTrackingSpecification: Record "Tracking Specification" temporary; ItemTrackingSetup: Record "Item Tracking Setup"): Boolean var
        JobPlanningLine: Record "Job Planning Line";
    begin
        if JobPlanningLine.Get(JobJnlLine."Job No.", JobJnlLine."Job Task No.", JobJnlLine."Job Planning Line No.")then begin
            TempTrackingSpecification.Reset();
            TempTrackingSpecification.SetSourceFilter(DATABASE::"Job Planning Line", 2, JobJnlLine."Job No.", JobPlanningLine."Job Contract Entry No.", false);
    ...
    begin
        TempGlobalAdjustEntry.Reset();
        if not TempGlobalAdjustEntry.FindSet()then exit;
        TempTrackingSpecification.Init();
        TempTrackingSpecification.Insert();
        repeat CreateEntrySummary(TempTrackingSpecification, TempGlobalAdjustEntry); // TrackingSpecification is a dummy record
            TempGlobalReservEntry:=TempGlobalAdjustEntry;
            TempGlobalReservEntry.Insert();
        until TempGlobalAdjustEntry.Next() = 0;
    end;
    local procedure MaxDoubleEntryAdjustQty(var TempItemTrackLineChanged: Record "Tracking Specification" temporary; var ChangedEntrySummary: Record "Entry Summary" temporary): Decimal var
        ItemJnlLine: Record "Item Journal Line";
        ItemTrackingSetup: Record "Item Tracking Setup";
    begin
        if not(TempItemTrackLineChanged."Source Type" = DATABASE::"Item Journal Line")then exit;
        if not(TempItemTrackLineChanged."Source Subtype" in[5, 6])then exit;
        if not ItemJnlLine.Get(TempItemTrackLineChanged."Source ID", TempItemTrackLineChanged."Source Batch Name", TempItemTrackLineChanged."Source Ref. No.")then exit;
        TempGlobalTrackingSpec.Reset();
        ItemTrackingSetup.CopyTrackingFromEntrySummary(ChangedEntrySummary);
        if FindRelatedParentTrkgSpec(ItemJnlLine, TempGlobalTrackingSpec, ItemTrackingSetup)then exit(-TempGlobalTrackingSpec."Quantity (Base)" - TempGlobalTrackingSpec."Buffer Value2");
    end;
    procedure CurrentDataSetMatches(ItemNo: Code[20]; VariantCode: Code[20]; LocationCode: Code[10]): Boolean begin
        exit((TempGlobalReservEntry."Item No." = ItemNo) and (TempGlobalReservEntry."Variant Code" = VariantCode) and (TempGlobalReservEntry."Location Code" = LocationCode));
    end;
    local procedure CheckJobInPurchLine(TrackingSpecification: Record "Tracking Specification"): Boolean var
        PurchLine: Record "Purchase Line";
    begin
        with TrackingSpecification do if("Source Type" = DATABASE::"Purchase Line") and ("Source Subtype" = "Source Subtype"::"3")then begin
                PurchLine.Reset();
                PurchLine.SetRange("Document Type", "Source Subtype");
                PurchLine.SetRange("Document No.", "Source ID");
                PurchLine.SetRange("Line No.", "Source Ref. No.");
                if PurchLine.FindFirst()then exit(PurchLine."Job No." <> '');
            end;
    end;
    procedure FindLotNoBySN(TrackingSpecification: Record "Tracking Specification"): Code[50]var
        LotNo: Code[50];
    begin
        if FindLotNoBySNSilent(LotNo, TrackingSpecification)then exit(LotNo);
        Error(LotNoBySNNotFoundErr, TrackingSpecification."Serial No.");
    end;
    procedure FindLotNoBySNSilent(var LotNo: Code[50]; TrackingSpecification: Record "Tracking Specification"): Boolean begin
        Clear(LotNo);
        if not(PartialGlobalDataSetExists or FullGlobalDataSetExists)then RetrieveLookupData(TrackingSpecification, true);
        TempGlobalEntrySummary.Reset();
        TempGlobalEntrySummary.SetTrackingKey();
        TempGlobalEntrySummary.SetRange("Serial No.", TrackingSpecification."Serial No.");
        TempGlobalEntrySummary.SetFilter("Lot No.", '<>%1', '');
        if not TempGlobalEntrySummary.FindFirst()then exit(false);
        LotNo:=TempGlobalEntrySummary."Lot No.";
        exit(true);
    end;
    procedure GetAvailableLotQty(TrackingSpecification: Record "Tracking Specification"): Decimal begin
        if TrackingSpecification."Lot No." = '' then exit(0);
        if not(PartialGlobalDataSetExists or FullGlobalDataSetExists)then RetrieveLookupData(TrackingSpecification, true);
        TempGlobalEntrySummary.Reset();
        TempGlobalEntrySummary.SetTrackingKey();
    ...
    begin
        ...
        if GlobalChangedEntrySummary.FindLast()then LastGlobalEntrySummary:=GlobalChangedEntrySummary;
    ...
    local procedure CanIncludeReservEntryToTrackingSpec(TempReservEntry: Record "Reservation Entry" temporary)Result: Boolean var
    ...
    begin
        ...
        if IsHandled then exit(Result);
        with TempReservEntry do if("Reservation Status" = "Reservation Status"::Prospect) and ("Source Type" = DATABASE::"Sales Line") and ("Source Subtype" = 2)then begin
                SalesLine.Get("Source Subtype", "Source ID", "Source Ref. No.");
                if SalesLine."Shipment No." <> '' then exit(false);
            end;
        exit(true);
    end;
